// New funnel: /3300us-Credit-Affiliate-Marketer and /3300us-Credit-Marketer
import { useState } from "react";
import { motion } from "framer-motion";
import {
  ArrowRight, CheckCircle2, Gift, Rocket, Shield, Sparkles, Star, TrendingUp, Users, Zap, ChevronDown, Quote, Share2, Target,
} from "lucide-react";
...
const PROBLEMS = [
  "You're sending traffic to generic affiliate links with zero personalization",
  "Your landing pages look identical to every other affiliate — no trust, no edge",
  "Email lists decay at 25%/year and you're not reactivating dormant subscribers",
  "You're spending on ads but losing money because your funnel doesn't convert cold traffic",
];

const HOW_STEPS = [
  { icon: Gift, title: "We Build Your Custom Branded Funnel", desc: "A full lead-capture funnel customized with YOUR branding and YOUR affiliate offers — designed to warm up cold traffic and build trust before the pitch." },
  { icon: Target, title: "Reactivate Your Email & SMS List", desc: "Our proven reactivation system re-engages your existing subscribers with personalized outreach that gets 2.7x more clicks than standard broadcast emails." },
  { icon: Share2, title: "Drive Viral Traffic With Gratitude Content", desc: "Repost viral gratitude & neuroscience clips from our Content Vault with your affiliate CTA. Build authority while driving warm traffic to your funnel." },
  { icon: Rocket, title: "2.7x More Conversions, Zero Extra Ad Spend", desc: "The same system behind 7 Inc 5000 companies. Smarter funnels, better follow-up, higher EPC — without increasing your ad budget." },
];

const TESTIMONIALS = [
  { name: "Sarah Mitchell", role: "Affiliate Marketer", niche: "Health & Wellness Offers", img: coachSarahImg, quote: "I was sending cold traffic straight to affiliate links and getting 0.5% conversion. With this funnel system, I warm them up first — now I'm converting at 4.2%. Same traffic, 8x the revenue.", results: [{ label: "EPC", before: "$0.12", after: "$0.97" }, { label: "Conv. rate", before: "0.5%", after: "4.2%" }, { label: "Revenue", before: "$2K/mo", after: "$14K/mo" }] },
  { name: "Marcus Chen", role: "Media Buyer & Affiliate", niche: "Digital Products", img: coachMarcusImg, quote: "The reactivation system alone paid for itself 10x over. I had 40K dormant email subscribers doing nothing. Reactivated 12K of them in the first week and made $8,200 in commissions.", results: [{ label: "List reactivated", before: "0", after: "12K" }, { label: "Open rate", before: "4%", after: "38%" }, { label: "Commissions", before: "$3K/mo", after: "$18K/mo" }] },
  { name: "Diana Rosales", role: "Content Affiliate", niche: "Personal Development", img: coachDianaImg, quote: "I post gratitude content from the vault and drive to my custom funnel. No more 'link in bio' to a boring landing page. People actually trust me now — and my conversions show it.", results: [{ label: "Clicks/day", before: "40", after: "220" }, { label: "Leads/day", before: "3", after: "28" }, { label: "Monthly rev", before: "$1.5K", after: "$9K" }] },
];

const VALUE_STACK = [
  { item: "The same proven system Inc 5000 companies paid $25,000 for", value: "$9,700" },
  { item: "Full branded funnel (customized with YOUR offers)", value: "$2,500" },
  { item: "List & subscriber reactivation system", value: "$1,500" },
  { item: "Automated email & SMS nurture sequences", value: "$1,500" },
  { item: "Viral content library with affiliate CTA overlays", value: "$1,000" },
  { item: "EPC optimization & conversion tracking", value: "$1,000" },
];

const FAQS = [
  { q: "How does this help my affiliate business?", a: "Instead of sending cold traffic to generic affiliate links, you warm them up through a branded funnel that builds trust. Warm leads convert 5-10x better than cold clicks. Same traffic, way higher EPC." },
  { q: "What if I promote multiple offers?", a: "Your funnel captures leads first, then segments them based on interest. You can promote any offer to a warm audience — health, biz-opp, digital products, whatever converts." },
  { q: "Do I need a big audience?", a: "No. The Content Vault gives you viral clips to repost. Build your audience while driving traffic. Many affiliates start with zero followers and scale to 10K+ in 90 days." },
...
export default function CreditAffiliateMarketer() {
  ...
  const { variant: abVariant, trackConversion: abTrack } = useABTest("credit_affiliate_headline");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("expert_leads").insert({
        full_name: name, email, niche: niche || "Affiliate Marketing", source_page: "3300us-Credit-Affiliate-Marketer",
      });
  ...
  return (
    <div className="min-h-screen bg-background">
      <section className="relative overflow-hidden"><div className="absolute inset-0 bg-gradient-to-b from-primary/5 via-transparent to-transparent" /><div className="relative max-w-3xl mx-auto px-4 pt-10 pb-16 text-center"><motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}><img src={logoImg} alt="I am Blessed AF" className="h-10 mx-auto mb-5" /><Badge className="mb-4 bg-primary/10 text-primary border-primary/20 text-xs font-bold px-3 py-1"><Sparkles className="w-3 h-3 mr-1" /> ATTENTION: AFFILIATE MARKETERS</Badge>
            {abVariant === "A" ? (
              <>
                <h1 className="text-3xl md:text-5xl font-black text-foreground leading-[1.1] mb-4 tracking-tight">A Neuroscience-Backed Viral <span className="text-primary">Gratitude Challenge</span><br className="hidden md:block" />That Converts 2–3x Better Than Cold Links 🧠</h1>
                <p className="text-base md:text-lg text-muted-foreground max-w-xl mx-auto mb-4 leading-relaxed">Replace generic affiliate links with a <strong className="text-foreground">science-backed gratitude funnel</strong> that warms up cold traffic and converts at <strong className="text-foreground">2–3x higher rates</strong> — customized with <strong className="text-foreground">YOUR affiliate offers</strong>.</p>
              ...
              <>
                <h1 className="text-3xl md:text-5xl font-black text-foreground leading-[1.1] mb-4 tracking-tight">Get a FREE <span className="text-primary">$3,300 Marketing Credit</span><br className="hidden md:block" />To 2.7x Your Affiliate Revenue 🚀</h1>
                <p className="text-base md:text-lg text-muted-foreground max-w-xl mx-auto mb-6 leading-relaxed">The <strong className="text-foreground">same strategy behind 7 Inc 5000 companies.</strong> A full funnel — <strong className="text-foreground">customized with YOUR affiliate offers</strong> — to warm up cold traffic, reactivate your list & convert at 2.7x higher rates.</p>
              </>
            )}</motion.div>
      ...
      <section id="enroll-section" className="bg-card border-y border-border/30"><div className="max-w-3xl mx-auto px-4 py-16"><motion.div className="max-w-md mx-auto" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}><div className="text-center mb-6"><Badge className="mb-3 bg-primary/10 text-primary border-primary/20 text-xs font-bold"><Rocket className="w-3 h-3 mr-1" /> LIMITED SPOTS</Badge><h2 className="text-2xl md:text-3xl font-bold text-foreground mb-2">Claim Your FREE <span className="text-primary">$3,300 Marketing Credit</span></h2><p className="text-sm text-muted-foreground">Full branded funnel to warm up traffic & 2.7x your affiliate conversions.</p></div>
        {enrolled ? (<motion.div className="bg-background border border-primary/20 rounded-2xl p-8 text-center" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}><CheckCircle2 className="w-12 h-12 text-primary mx-auto mb-3" /><h3 className="text-lg font-bold text-foreground mb-1">You're In! 🎉</h3><p className="text-sm text-muted-foreground">Check your email — your custom funnel will be live within 48 hours.</p></motion.div>) : (
          <form onSubmit={handleSubmit} className="bg-background border border-border/40 rounded-2xl p-6 space-y-4">
            ...
            <div><label className="text-xs font-medium text-muted-foreground mb-1 block">Your Niche (optional)</label><Input value={niche} onChange={(e) => setNiche(e.target.value)} placeholder="e.g. Health, Biz-Opp, Digital Products" className="h-11" /></div>
      ...
      <section className="bg-foreground"><div className="max-w-3xl mx-auto px-4 py-14 text-center"><h2 className="text-2xl md:text-3xl font-bold text-background mb-3">Stop Sending Cold Traffic. Start <span className="text-primary">Converting.</span></h2><p className="text-sm text-background/70 max-w-md mx-auto mb-6">While other affiliates fight over the same cold audience, you'll have warm funnels converting at 2.7x.</p><Button size="lg" onClick={scrollToEnroll} className="bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-base px-8 py-6 rounded-xl gap-2">Claim My FREE $3,300 Credit <ArrowRight className="w-5 h-5" /></Button></div></section>
