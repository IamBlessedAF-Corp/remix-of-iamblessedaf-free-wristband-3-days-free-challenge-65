// Renamed from NetworkMarketers — route: /3300us-Credit-N-Marketer
import { useState } from "react";
import { motion } from "framer-motion";
import {
  ArrowRight, CheckCircle2, Gift, Rocket, Shield, Sparkles, Star, TrendingUp, Users, Zap, ChevronDown, Quote, Share2, Phone,
} from "lucide-react";
...
const HOW_STEPS = [
  { icon: Gift, title: "We Build Your Custom Branded Funnel", desc: "A full lead-capture funnel customized with YOUR branding and YOUR products — designed to attract warm prospects instead of cold pitching." },
  { icon: Phone, title: "Capture Emails & Phone Numbers Automatically", desc: "Every interested prospect enters their contact info through your funnel. You get a warm lead list of people who already said YES." },
  { icon: Rocket, title: "Start Conversations That Lead to Sales", desc: "Follow up naturally with warm prospects who already know you. Now you have a real relationship — and a warm prospect for your products." },
];

const TESTIMONIALS = [
  { name: "Sarah Mitchell", role: "Health & Wellness Rep", niche: "Essential Oils & Supplements", img: coachSarahImg, quote: "I used to send 50 cold DMs a day with maybe 2 replies. Now I post value content, drive people to my funnel, and get 15-20 warm conversations DAILY. My team duplication went through the roof.", results: [{ label: "DM replies", before: "2/day", after: "18/day" }, { label: "Team signups", before: "2/mo", after: "9/mo" }, { label: "Income", before: "$800", after: "$4,200" }] },
  { name: "Marcus Chen", role: "Fitness Network Leader", niche: "Nutrition & Performance", img: coachMarcusImg, quote: "This funnel system is the ultimate ice-breaker. I share value content and drive traffic to my branded funnel — the DMs flood in from people who already trust me.", results: [{ label: "Leads/week", before: "8", after: "35" }, { label: "Conversions", before: "3%", after: "22%" }, { label: "Team size", before: "12", after: "47" }] },
  { name: "Diana Rosales", role: "Beauty & Skincare Rep", niche: "Clean Beauty MLM", img: coachDianaImg, quote: "My Instagram stories used to feel so salesy. Now I share value-first content, drive to my funnel, and people come to ME asking about my products. The energy completely shifted.", results: [{ label: "Story replies", before: "5/wk", after: "40/wk" }, { label: "New customers", before: "4/mo", after: "16/mo" }, { label: "Referrals", before: "1/mo", after: "8/mo" }] },
];
...
export default function CreditNMarketer() {
  ...
  const { variant: abVariant, trackConversion: abTrack } = useABTest("credit_nmarketer_headline");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("expert_leads").insert({
        full_name: name, email, niche: niche || "Network Marketing", source_page: "3300us-Credit-N-Marketer",
      });
      if (error) throw error;
      abTrack();
      supabase.functions.invoke("send-network-marketer-welcome", { body: { email, name, niche: niche || "Network Marketing" } }).catch(console.error);
      setEnrolled(true);
  ...
  return (
    <div className="min-h-screen bg-background">
      <section className="relative overflow-hidden"><div className="absolute inset-0 bg-gradient-to-b from-primary/5 via-transparent to-transparent" /><div className="relative max-w-3xl mx-auto px-4 pt-10 pb-16 text-center"><motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}><img src={logoImg} alt="I am Blessed AF" className="h-10 mx-auto mb-5" /><Badge className="mb-4 bg-primary/10 text-primary border-primary/20 text-xs font-bold px-3 py-1"><Sparkles className="w-3 h-3 mr-1" /> ATTENTION: NETWORK MARKETERS & SOCIAL SELLERS</Badge>
            {abVariant === "A" ? (
              <>
                <h1 className="text-3xl md:text-5xl font-black text-foreground leading-[1.1] mb-4 tracking-tight">A Neuroscience-Backed Viral <span className="text-primary">Gratitude Challenge</span><br className="hidden md:block" />That Captures 2–3x More Warm Leads 🧠</h1>
                <p className="text-base md:text-lg text-muted-foreground max-w-xl mx-auto mb-4 leading-relaxed">Replace cold DMs with a <strong className="text-foreground">science-backed gratitude challenge</strong> that turns strangers into <strong className="text-foreground">warm conversations</strong> — customized with <strong className="text-foreground">YOUR branding</strong>.</p>
              ...
              <>
                ...
                <p className="text-base md:text-lg text-muted-foreground max-w-xl mx-auto mb-6 leading-relaxed">The <strong className="text-foreground">same strategy we used to increase 2.7x lead capture for 7 Inc 5000 companies.</strong> A full funnel — <strong className="text-foreground">customized with YOUR branding</strong> — to reactivate your list, re-engage past clients & start <strong className="text-foreground">real conversations that close sales.</strong></p>
              </>
            )}</motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}><Button size="lg" onClick={scrollToEnroll} className="bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-base px-8 py-6 rounded-xl gap-2 btn-glow">YES! Claim My FREE $3,300 Marketing Credit <ArrowRight className="w-5 h-5" /></Button><p className="text-xs text-muted-foreground mt-2">100% free. Same strategy behind 7 Inc 5000 companies. Works with any network or product.</p></motion.div>
      ...
      <section id="enroll-section" className="bg-card border-y border-border/30"><div className="max-w-3xl mx-auto px-4 py-16"><motion.div className="max-w-md mx-auto" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}><div className="text-center mb-6"><Badge className="mb-3 bg-primary/10 text-primary border-primary/20 text-xs font-bold"><Rocket className="w-3 h-3 mr-1" /> FIRST 100 MARKETERS ONLY</Badge><h2 className="text-2xl md:text-3xl font-bold text-foreground mb-2">Claim Your FREE <span className="text-primary">$3,300 Marketing Credit</span></h2><p className="text-sm text-muted-foreground">Full branded funnel to reactivate your list & past clients. Same strategy behind 2.7x lead capture for 7 Inc 5000 companies.</p></div>
        {enrolled ? (<motion.div className="bg-background border border-primary/20 rounded-2xl p-8 text-center" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}><CheckCircle2 className="w-12 h-12 text-primary mx-auto mb-3" /><h3 className="text-lg font-bold text-foreground mb-1">You're In! 🎉</h3><p className="text-sm text-muted-foreground">Check your email for your funnel access, DM scripts & content library.</p></motion.div>) : (
          <form onSubmit={handleSubmit} className="bg-background border border-border/40 rounded-2xl p-6 space-y-4">
            ...
            <div><label className="text-xs font-medium text-muted-foreground mb-1 block">Your Network/Product (optional)</label><Input value={niche} onChange={(e) => setNiche(e.target.value)} placeholder="e.g. Health & Wellness, Beauty, Fitness" className="h-11" /></div>
            ...
            <div className="flex items-center justify-center gap-4 text-[10px] text-muted-foreground"><span className="flex items-center gap-1"><Shield className="w-3 h-3" /> No credit card</span><span className="flex items-center gap-1"><Zap className="w-3 h-3" /> Instant access</span><span className="flex items-center gap-1"><Users className="w-3 h-3" /> Works with any network</span></div>
          </form>
      ...
      <section className="bg-foreground"><div className="max-w-3xl mx-auto px-4 py-14 text-center"><h2 className="text-2xl md:text-3xl font-bold text-background mb-3">Stop Cold DM'ing. Start <span className="text-primary">Attracting.</span></h2><p className="text-sm text-background/70 max-w-md mx-auto mb-6">While other reps fight over the same cold audience, you'll have warm prospects coming to YOU.</p><Button size="lg" onClick={scrollToEnroll} className="bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-base px-8 py-6 rounded-xl gap-2">Claim My FREE $3,300 Credit <ArrowRight className="w-5 h-5" /></Button></div></section>
