...
import { RefreshCw, Pencil, CheckCircle, X, Users } from "lucide-react";
...
export default function ChallengeTab() {
  ...
  const avgStreak = participants.length > 0
    ? Math.round(participants.reduce((s, p) => s + (p.current_streak || 0), 0) / participants.length)
    : 0;
  ...
  return (
    <div className="space-y-4">
      <AdminSectionDashboard
        ...
        description="Challenge participants, streaks, and activation status"
        kpis={[
          { label: "Total Participants", value: participants.length },
          { label: "Active", value: active },
          { label: "Completed", value: completed },
          { label: "Avg Streak", value: `${avgStreak} 🔥` },
        ]}
      />
