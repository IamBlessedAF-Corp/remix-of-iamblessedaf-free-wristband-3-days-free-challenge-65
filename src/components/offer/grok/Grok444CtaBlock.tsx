...
const Grok444CtaBlock = ({
  onCheckout,
  delay = 0,
  showScarcity = false,
  loading = false,
}: Grok444CtaBlockProps) => {
    ...
    >
      {showScarcity && (
        <div className="bg-primary/10 border border-primary/20 rounded-xl p-3 mb-3 text-center">
          <p className="text-xs font-semibold text-primary uppercase tracking-wider">
            ⚡ Only 44 Habit Lock Packs available this batch — 1,111 meals waiting to be donated
          </p>
        </div>
      )}
      ...
      <div className="text-center mb-3" data-price-anchor>
        <div className="flex items-center justify-center gap-3">
          <span className="text-sm text-muted-foreground line-through">$888</span>
          <span className="text-3xl font-black text-foreground">$444</span>
      ...
      >
        ...
        {loading ? "Creating checkout…" : utmCta.showDiscount
          ? "YES! Claim My $444 Habit Lock Pack →"
          : "YES! Feed 1,111 People & Lock In My $444 Pack!"}
...
export default Grok444CtaBlock;
